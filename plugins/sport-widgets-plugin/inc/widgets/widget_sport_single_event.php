	...
	class sport_single_event extends WP_Widget {
		...
		***************************************/
		function __construct () {
				...
				parent::__construct('sport_single_event', __('Sport: Single Event', "loc_sport_widgets_plugin"), $widget_ops, $control_ops );	
		}
		...
		***************************************/
		function form($instance) {
			...
			//defaults
			$defaults = array( 
				'title' 			=> __('Event', "loc_sport_widgets_plugin"),
				'excerpt_length' 	=> 650,
			);
			...
			?>
				...
				<?php 
					...
					<!-- DYNAMIC SELECT -->
						<?php 
						 ...
						 ?>
						<p>
							<label><?php _e("Select event", "loc_sport_widgets_plugin"); ?>: </label><br>
				...
				<p>
					<label for='<?php echo esc_attr($this->get_field_id('excerpt_length')); ?>'><?php _e("Automatic Excerpt Size", "loc_sport_widgets_plugin"); ?>	: </label><br>
					<input 
						style='width: 80px;'
						type='number' 
						min='0'
		...
		***************************************/
		function widget($args, $instance) {
			...
			if (function_exists('icl_translate') && function_exists('icl_register_string')) {
				...
				// VERSION < 3.3
				icl_register_string ('loc_sport_widgets_plugin', "$widget_id-widget[cost]", $cost);
				...
				// VERSION > v3.3
				do_action('wpml_register_single_string', 'loc_sport_widgets_plugin', "$widget_id-widget[cost]", $cost);
				
				$cost = apply_filters('wpml_translate_single_string', $cost, 'loc_sport_widgets_plugin', "$widget_id-widget[cost]");
