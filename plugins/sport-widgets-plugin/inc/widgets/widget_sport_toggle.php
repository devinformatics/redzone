	...
	class sport_toggle extends WP_Widget {
		...
		***************************************/
		function __construct () {
				...
				parent::__construct('sport_toggle', __('Sport: Toggle', "loc_sport_widgets_plugin"), $widget_ops, $control_ops );	
		}
		...
		***************************************/
		function form($instance) {
			...
			//defaults
			$defaults = array( 
				'title' 		=> __('Toggle', "loc_sport_widgets_plugin"),
				'toggle'		=> array(
					0				=> array(
    				...
    				<?php
    					...
    					for ($i = 0; $i < count($toggle); $i++) {  
    						...
    						if (function_exists('icl_translate') && function_exists('icl_register_string')) {
					            ...
					            // VERSION < 3.3
					            icl_register_string ('loc_sport_widgets_plugin', "$widget_id-widget[$i][title]", $toggle[$i]['title']);
					            icl_register_string ('loc_sport_widgets_plugin', "$widget_id-widget[$i][content]", $toggle[$i]['content']);
					            ...
					            // VERSION > v3.3
					            do_action('wpml_register_single_string', 'loc_sport_widgets_plugin', "$widget_id-widget[$i][title]", $toggle[$i]['title']);
					            do_action('wpml_register_single_string', 'loc_sport_widgets_plugin', "$widget_id-widget[$i][content]", $toggle[$i]['content']);
					            
					            $toggle[$i]['title'] = apply_filters('wpml_translate_single_string', $toggle[$i]['title'], 'loc_sport_widgets_plugin', "$widget_id-widget[$i][title]");
					            $toggle[$i]['content'] = apply_filters('wpml_translate_single_string', $toggle[$i]['content'], 'loc_sport_widgets_plugin', "$widget_id-widget[$i][content]");
