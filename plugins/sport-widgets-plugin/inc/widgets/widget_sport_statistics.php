...
/**************************************
WIDGET: sport_statistics
***************************************/

	add_action('widgets_init', 'register_widget_sport_statistics' );
	function register_widget_sport_statistics () {
		register_widget('sport_statistics');	
	}

	class sport_statistics extends WP_Widget {
		...
		***************************************/
		function __construct () {
				...
				$widget_ops = array(
					'classname' => 'sport_statistics', 								
					'description' => __('Display statistics', "loc_sport_widgets_plugin")	 				
				);
				$control_ops = array(
					'width' => 550, 
					'height' => 350, 
					'id_base' => 'sport_statistics' 														
				);

				parent::__construct('sport_statistics', __('Sport: Statistics', "loc_sport_widgets_plugin"), $widget_ops, $control_ops );	
		}
		...
		***************************************/
		function form($instance) {
			...
			//defaults
			$defaults = array( 
				'title' 		=> __('Statistics', "loc_sport_widgets_plugin"),
				'statistic'		=> array(
					0 				=> array(
						'icon'			=> 'fa-flag',
			...
			?>
				...
				<p>
					<label for='<?php echo esc_attr($this->get_field_id('text')); ?>'><?php _e("Statistics text", "loc_sport_widgets_plugin"); ?></label><br>
				...
				<p>
					<label for="<?php echo esc_attr($this->get_field_id('read_more_link')); ?> "><?php _e("Read More Link (Optional)", "loc_sport_widgets_plugin"); ?>:</label><br> 
				...
				<?php 
					...
					for ($i = 0; $i < count($statistic); $i++) {  
						...
						if (function_exists('icl_translate') && function_exists('icl_register_string')) {
				            ...
				            // VERSION < 3.3
				            icl_register_string ('loc_sport_widgets_plugin', "$widget_id-widget[$i][stat_text]", $statistic[$i]['stat_text']);
				            icl_register_string ('loc_sport_widgets_plugin', "$widget_id-widget[$i][stat_num]", $statistic[$i]['stat_num']);

				            $statistic[$i]['stat_text'] = icl_translate('loc_sport_widgets_plugin', "$widget_id-widget[$i][stat_text]", $statistic[$i]['stat_text']);
				            $statistic[$i]['stat_num'] = icl_translate('loc_sport_widgets_plugin', "$widget_id-widget[$i][stat_num]", $statistic[$i]['stat_num']);
						
						} elseif (class_exists('SitePress')) {
				            
				            // VERSION > v3.3
				            do_action('wpml_register_single_string', 'loc_sport_widgets_plugin', "$widget_id-widget[$i][stat_text]", $statistic[$i]['stat_text']);
				            do_action('wpml_register_single_string', 'loc_sport_widgets_plugin', "$widget_id-widget[$i][stat_num]", $statistic[$i]['stat_num']);
				            
				            $statistic[$i]['stat_text'] = apply_filters('wpml_translate_single_string', $statistic[$i]['stat_text'], 'loc_sport_widgets_plugin', "$widget_id-widget[$i][stat_text]");
				            $statistic[$i]['stat_num'] = apply_filters('wpml_translate_single_string', $statistic[$i]['stat_num'], 'loc_sport_widgets_plugin', "$widget_id-widget[$i][stat_num]");
